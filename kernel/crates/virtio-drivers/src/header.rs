use crate::PAGE_SIZE;
use bitflags::*;
use super::mmio_access::{ReadOnly, WriteOnly, Volatile};
use bitflags;
...
impl VirtIOHeader {
    ...
    pub fn begin_init(&mut self, negotiate_features: impl FnOnce(u64) -> u64) {
        self.status.write(DeviceStatus::ACKNOWLEDGE.bits());
        self.status.write(DeviceStatus::DRIVER.bits());

        let features = self.read_device_features();
        self.write_driver_features(negotiate_features(features));
        self.status.write(DeviceStatus::FEATURES_OK.bits());
    ...
    pub fn finish_init(&mut self) {
        self.status.write(DeviceStatus::DRIVER_OK.bits());
    }
